 ...
 */
public class MyBaseBindingRecyclerViewAdapter<M, B extends ViewDataBinding> extends RecyclerView.Adapter<MyBaseBindingViewHolder<B>> {
     ...
     */
    public MyBaseBindingRecyclerViewAdapter() {
    }
     ...
     */
    public MyBaseBindingRecyclerViewAdapter(int defaultItemLayoutId) {
    ...
    public int getItemViewType(int position) {
        if (mDefaultItemLayoutId == 0) {
            throw new RuntimeException("请在 " + this.getClass().getSimpleName() + " 中重写 getItemViewType 方法返回布局资源 id，或者使用 MyBaseBindingRecyclerViewAdapter 一个参数的构造方法 MyBaseBindingRecyclerViewAdapter(int defaultItemLayoutId)");
        }
        return mDefaultItemLayoutId;
    }

    @Override
    public MyBaseBindingViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new MyBaseBindingViewHolder(this, DataBindingUtil
            .inflate(getLayoutInflater(parent), viewType, parent, false));
    }

    @Override
    public void onBindViewHolder(MyBaseBindingViewHolder<B> viewHolder, int position) {
        // 在设置值的过程中忽略选中状态变化
    ...
    public MyBaseHeaderAndFooterAdapter getHeaderAndFooterAdapter() {
        if (mHeaderAndFooterAdapter == null) {
            synchronized (MyBaseBindingRecyclerViewAdapter.this) {
                if (mHeaderAndFooterAdapter == null) {
