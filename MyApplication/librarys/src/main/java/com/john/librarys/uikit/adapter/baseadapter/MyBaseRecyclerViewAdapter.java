 ...
 * @param <M> 适配的数据类型
 */
public abstract class MyBaseRecyclerViewAdapter<M> extends RecyclerView.Adapter<MyBaseRecyclerViewHolder> {
    ...
    protected MyBaseHeaderAndFooterAdapter mHeaderAndFooterAdapter;
    ...
    public MyBaseRecyclerViewAdapter(RecyclerView recyclerView) {
        mRecyclerView = recyclerView;
        mContext = mRecyclerView.getContext();
        mData = new ArrayList<>();
    }

    public MyBaseRecyclerViewAdapter(RecyclerView recyclerView, int defaultItemLayoutId) {
        this(recyclerView);
    ...
    public MyBaseRecyclerViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        MyBaseRecyclerViewHolder viewHolder = new MyBaseRecyclerViewHolder(this, mRecyclerView, LayoutInflater.from(mContext).inflate(viewType, parent, false), mOnRVItemClickListener, mOnRVItemLongClickListener);
    ...
    public int getItemViewType(int position) {
        if (mDefaultItemLayoutId == 0) {
            throw new RuntimeException("请在 " + this.getClass().getSimpleName() + " 中重写 getItemViewType 方法返回布局资源 id，或者使用 MyBaseRecyclerViewAdapter 两个参数的构造方法 MyBaseRecyclerViewAdapter(RecyclerView recyclerView, int itemLayoutId)");
        }
        return mDefaultItemLayoutId;
    }

    @Override
    public void onBindViewHolder(MyBaseRecyclerViewHolder viewHolder, int position) {
        // 在设置值的过程中忽略选中状态变化
     ...
     * @param model
     */
    protected abstract void fillData(MyBaseViewHolderHelper helper, int position, M model);
     ...
     */
    public void setOnRVItemClickListener(MyBaseOnRVItemClickListener onRVItemClickListener) {
     ...
     */
    public void setOnRVItemLongClickListener(MyBaseOnRVItemLongClickListener onRVItemLongClickListener) {
    ...
    public MyBaseHeaderAndFooterAdapter getHeaderAndFooterAdapter() {
        if (mHeaderAndFooterAdapter == null) {
            synchronized (MyBaseRecyclerViewAdapter.this) {
                if (mHeaderAndFooterAdapter == null) {
                    mHeaderAndFooterAdapter = new MyBaseHeaderAndFooterAdapter(this);
                }
            }
        }
