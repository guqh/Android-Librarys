...
import com.john.librarys.pulltorefresh.internal.FlipLoadingLayout;
import com.john.librarys.pulltorefresh.internal.LoadingLayout;
import com.john.librarys.pulltorefresh.internal.RotateLoadingLayout;
import com.john.librarys.pulltorefresh.internal.Utils;
import com.john.librarys.pulltorefresh.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    public final void onRefreshComplete() {
        if (isRefreshing()) {
            //fix bug 发现个奇怪的bug，当下来刷新数据的时候状态栏出来了，但是马上接着调用 onRefreshComplete ，状态栏不会消失
            //必须要post 后才会生效？？？
            post(new Runnable() {
                @Override
                public void run() {
    ...
    /**
     * 强制刷新，可以根据 mode 的值来刷新
     * 但是mode 只能取 Mode.PULL_FROM_END | Mode.PULL_FROM_START,其他值无效。
     *
     * @param doScroll
     * @param mode
     */
    public final void setRefreshing(boolean doScroll, Mode mode) {
        if (!isRefreshing()) {
            switch (mode) {
